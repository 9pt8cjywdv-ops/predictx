    ...
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <h2 className="modal-title" style={{ marginBottom: 0 }}>{isLogin ? 'Welcome back' : 'Create account'}</h2>
        ...
        {!isLogin && (
          <div style={{ background: 'rgba(0,211,149,0.06)', border: '1px solid rgba(0,211,149,0.2)', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: '0.85rem', color: 'var(--green)' }}>
            🎁 New users start with <strong>ZAR 1,000</strong> in play money
          </div>
        )}
        ...
        <div style={{ textAlign: 'center', marginTop: 16, fontSize: '0.85rem', color: 'var(--text2)' }}>
          ...
          <span style={{ color: 'var(--accent)', cursor: 'pointer', fontWeight: 700 }} onClick={() => setAuthModal(isLogin ? 'register' : 'login')}>
